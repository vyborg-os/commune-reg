            ...
            if(empty($firstname) || empty($lastname) || empty($captcha)  || empty($email) || empty($password) || empty($cpassword) ){
                ...
                if($insert->affected_rows > 0){
                    ...
                    //$snd = mail($email,$subject,$otp);
                        $mail = new PHPMailer();
                        $mail->isSMTP();
                        $mail->Host = 'grezitioninter.com.ng';
                        $mail->SMTPAuth = true;
                        $mail->Username = 'commune@grezitioninter.com.ng';
                        $mail->Password = 'Programmer$$01';
                        $mail->SMTPSecure = 'tls';
                        $mail->Port = 587;
                        $mail->setFrom('commune@grezitioninter.com.ng', 'Commune');
                        $mail->addAddress($to);
                        $mail->isHTML(true);
